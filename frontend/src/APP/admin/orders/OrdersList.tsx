                  ...
                  <tr key={order._id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button
                        ...
                        onClick={async () => {
                          if (window.confirm('Are you sure you want to delete this order? This will soft-delete the order and mark it as cancelled.')) {
                            const token = localStorage.getItem('adminToken');
                            if (!token) {
                              alert('No authentication token found. Please log in again.');
                              return;
            ...
            {Math.ceil(orders.length / rowsPerPage) > 1 && (
              <Stack spacing={2} sx={{ mt: 3, mb: 2, alignItems: 'center' }}>
                <Pagination
                  ...
                  hidePrevButton={orders.length <= rowsPerPage}
                  hideNextButton={orders.length <= rowsPerPage}
                />
              </Stack>
            )}
          </div>
