  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        setProducts(data.filter((p: Product) => p.category?.toLowerCase().includes('camera')));
      } catch (err) {
        setProducts([]);
      } finally {
        setLoading(false);
    ...
    <div className="min-h-screen bg-[#181622] text-white">
      ...
      <div className="max-w-7xl mx-auto py-10 px-4 gap-8">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex items-center space-x-2">
              <span className="text-gray-400 text-sm">
                {filteredProducts.length} cameras found
              </span>
        ...
        <main className="flex-1">
          <h1 className="text-3xl font-bold mb-8">Popular Cameras</h1>
          {loading ? (
            <div className="text-center text-gray-400">Loading...</div>
          ) : filteredProducts.length === 0 ? (
            <div className="text-center text-gray-400">No cameras found.</div>
          ) : (
            viewMode === 'grid' ? (
